...
class Login(APIView):
	def generate_token(self, profile_id, anonymous, admin):
		payload = {
	...
	def post(self, request):
		try:
			...
			if data["mode"] == "profile":
				...
				if user != None:
					token = self.generate_token(Profile.objects.get(email=data["email"]).id, False, user.is_superuser)
					result = {"token": token}
				...
				token = self.generate_token(profile_id=profile.id, anonymous=True, admin=False)
				result = {"token": token}
				return Response(result, status=status.HTTP_200_OK)
			else:
